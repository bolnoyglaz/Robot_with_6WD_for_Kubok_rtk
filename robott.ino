...
#define DATCHIK_IN_PIN  19  //

// These bit flags are set in bUpdateFlagsShared to indicate which
// channels have new signals
#define THROTTLE_FLAG 1
#define STEERING_FLAG 2
#define DATCHIK_FLAG 5
...
volatile uint16_t unDatchikInShared;
...
void setup()
{
  ...
  attachInterrupt(4 /* INT4 = DATCHIK_IN_PIN */, calcDatchik, CHANGE);
...
/*void calcDatchik()
  {
  if (digitalRead(DATCHIK_IN_PIN) == HIGH)
  {
    gDatchik = DATCHIK_UP;
  }
  else
  {
    gDatchik = DATCHIK_DOWN         ;          // unDatchikInShared = (uint16_t)(micros() - ulDatchikStart);
    //bUpdateFlagsShared |= DATCHIK_FLAG;
  }
  }  */
void calcDatchik()
{
  if (digitalRead(DATCHIK_IN_PIN) == HIGH)
  {
    ulDatchikStart = micros();
  }
  else
  {
    unDatchikInShared = (uint16_t)(micros() - ulDatchikStart);
    bUpdateFlagsShared |= DATCHIK_FLAG;
  }
}
